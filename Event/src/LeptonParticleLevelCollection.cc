...
LeptonParticleLevelCollection::size_type LeptonParticleLevelCollection::WLeptonIndex(){
    // get the index of the lepton from a W boson
    // (only intended for three-lepton signatures with one Z boson candidate)
    initializeZBosonCandidate();
    std::pair< size_type, size_type > zinds = bestZBosonCandidateIndices();
    // loop over indices (leptons are already sorted in Z boson initialization),
    // if index does not belong to Z candidate, it is considered the W boson lepton.
    for(unsigned int idx = 0; idx<size(); idx++){
	if( idx!=zinds.first && idx!=zinds.second ) return idx;
    }
    return 99;
}
