...
class LeptonParticleLevelCollection : public PhysicsObjectCollection< LeptonParticleLevel > {
	...
	bool hasZTollCandidate(double halfWindow, 
                               ...
                               bool force = false);
	std::pair< size_type, size_type > bestZBosonCandidateIndices() const{ return _bestZBosonCandidateIndices; }
	
	// W boson reconstruction
	// (only works in three-lepton events with a Z boson candidate)
	size_type WLeptonIndex();
